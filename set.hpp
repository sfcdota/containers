...
//    value_compare vc;
 public:
  ...
  set(const set &x): allocator_(x.allocator_), root_(NULL), size_(0), comp_(x.comp_), vc(comp_) {
    end_ = NewNode();
    end_->parent = NULL;
    end_->left = end_;
    end_->right = end_;
  ...
  //todo delete end_
  ~set() { clear(); if (end_) DeleteNode(end_); };
...
//        right = tmp;
  ...
  friend bool operator!= ( const set<T,Compare,Alloc>& lhs,
                           const set<T,Compare,Alloc>& rhs ) { return !(lhs == rhs); };
  friend bool operator<  ( const set<T,Compare,Alloc>& lhs,
                           const set<T,Compare,Alloc>& rhs ) { return lexicographical_compare(lhs.begin(), lhs.end(), rhs.begin(), rhs.end()); };
  friend bool operator<= ( const set<T,Compare,Alloc>& lhs,
                           const set<T,Compare,Alloc>& rhs ) { return !(rhs < lhs); }
  friend bool operator>  ( const set<T,Compare,Alloc>& lhs,
                           const set<T,Compare,Alloc>& rhs ) { return rhs < lhs; };
  friend bool operator>= ( const set<T,Compare,Alloc>& lhs,
                           const set<T,Compare,Alloc>& rhs ) { return !(lhs < rhs); }

  friend  void swap (set<T,Compare,Alloc>& x, set<T,Compare,Alloc>& y) { x.swap(y); }
};
}
#endif
